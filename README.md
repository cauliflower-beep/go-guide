...
- 一类是通过反射在运行时进行依赖注入，典型代表是 uber 开源的[dig](https://link.zhihu.com/?target=https%3A//github.com/uber-go/dig)和Facebook的 [inject] ([https://github.com/facebookarchive/inject](https://link.zhihu.com/?target=https%3A//github.com/facebookarchive/inject))
...
**Injector:** 由`wire`自动生成的函数。函数内部会按根据依赖顺序调用相关privoder 。

为了生成此函数， 我们在 `wire.go` (文件名非强制，但一般约定如此)文件中定义injector函数签名。 然后在函数体中调用`wire.Build` ，并以所需provider作为参数（无须考虑顺序）。

由于`wire.go`中的函数并没有真正返回值，为避免编译器报错， 简单地用`panic`函数包装起来即可。不用担心执行时报错， 因为它不会实际运行，只是用来生成真正的代码的依据。一个简单的wire.go 示例
...
了解了依赖注入，便可依赖其避免可变全局变量的出现。示例代码如下：
...
假设使用共享的 `AbstractList` 实现了多种列表类型，需要避免在具体的列表实现中嵌入 `AbstractList`。
...
尽可能指定容器容量，以便为容器预先分配内存。这将帮助在添加元素时最小化后续分配的开销（通过复制和调整容器大小）。
...
向`make()`提供容量提示，会在初始化时尝试调整 map 的大小，这将减少在将元素添加到 map 时为 map 重新分配内存。
...
我们建议将行长度限制为 **99 characters** (99 个字符)。展示在goland等ide中就是不要超过工作区竖直的线。

coder应该在达到这个限制之前换行。但这不是硬性限制。允许代码超过此限制。
...
相反，在一个代码库中包含多个完全不同或冲突的代码风格，会导致维护成本开销、不确定性和认知偏差。所有这些都会直接导致速度降低、代码审查痛苦、而且增加 bug 数量。
...
这将零值结构体与那些具有类似于为 [初始化 Maps](#初始化-maps) 创建的，区别于非零值字段的结构区分开来，并与我们更喜欢的 [声明空切片] 方式相匹配。
...
// Good
sval := T{Name: "foo"} // 结构体初始化

sptr := &T{Name: "bar"} // 结构体指针初始化
```
